## Prometurbo Deploy via Helm Charts

Review the prerequisites defined in the [Deploy Overview](https://github.com/turbonomic/prometurbo/blob/master/deploy/README.md)
...
The following table shows the values exposed which are also seen in the file [values.yaml](https://github.com/turbonomic/prometurbo/blob/master/deploy/prometurbo/values.yaml), and values that are default and/or required are noted as such.

Parameter|Default Value|Required / Opt to Change|Parameter Type
------------ | ------------- | --------------- | -------------
image.repository|turbonomic/prometurbo|optional|path to repo
image.tag|6.4dev|optional|prometurbo tag
...
NOTE: Starting with prometurbo 6.3+, you do not need to make this configMap modification if updating to a minor version like 6.3.0 -> 6.3.1, which will now be automatically handled.  You would only need to make this change if you are making a major change, going from 6.3.x -> 6.4.x, or 6.3.x -> 7.x.
...
1. Insure prometurbo pod restarted to pick up new value
1. Repeat for every kubernetes / OpenShift cluster with a prometurbo pod

#### Updating Prometurbo Image
You may be instructed to update the prometurbo pod image.  Determine which new tag you will be using.  You will either be instructed by Turbonomic to use a new image, or you may want to refresh the image to pick up a security patch.
...
1. Insure prometurbo pod redeployed with new image
1. Repeat for every kubernetes / OpenShift cluster with a prometurbo pod

There's no place like home... go back to the [Turbonomic Overview](https://github.com/turbonomic/prometurbo/blob/master/README.md) or the [Deploy Overview](https://github.com/turbonomic/prometurbo/blob/master/deploy/README.md).